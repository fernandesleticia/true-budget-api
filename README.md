...
**to make source code changes inside the container also persist, use a second volume pointing to the source code - this is called Bind Mounts**

**volume for the source code starts with the path to the source code in your machine and after the : the working directoty folder**

**the path to the source code in the local machine must the the absolut path to the project folder, not the relative one**

**using quotes is optional, but got to avoid issues when the path has special charateres or white space**

**we can also map only specifc files, no need to me the whole project**

**Make sure docker has access to the folder which your sharing as bind mount - if you have problems with file sharing, It depends on how you setup docker (docker toolbox)**

**when we do *-v "/path/to/source/code/on/the/host/machine/my_project:/app*, we override the app workdir (we makes resources generated by the RUN command inexistent in the app - like node_modules or Gemfile.lock), so we need to tell docker that some parts should not be overwritten (that cab be achieve adding another volume - an anonymous volume: *-v /app/node_modules* (we could that with VOLUME [ "/app/node_modules" ] in the Dockerfile too))**

Docker evaluates all volumes you're setting in a container, and the longer more specifc path have priority. That's why the anonymous app/node_modules overwrites the node_modules comming from the previous app volume mapping, and the node_modules will remain on the anonymous volume.

```
docker run -p 3000:3000 -d --name app_name --rm -v volume_name:/app/path/to/container/folder -v "/path/to/source/code/on/the/host/machine/my_project:/app" -v /app/node_modules demo:latest
```
