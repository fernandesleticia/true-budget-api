...
RSpec.describe "/wallets", type: :request do
  ...
  context "show" do
    ...
    let(:path) { wallets_path(wallet.id) }
    ...
    it "should return the wallet" do
      ...
      expect(JSON.parse(response.body)[0]).to eq(wallet.to_hash)
    end
  end
  ...
  context "index" do
    ...
    it "should return the wallets" do
      subject

      expect(response.status).to eq 200
      expect(JSON.parse(response.body)).to match_array(Wallet.serialize(wallets))
    end
  end
end
